...
import { ArrowLeft, Compass, DoorOpen, GitCompare, Layers, Sparkles, TrendingDown } from "lucide-react";
...
export default function DebtPayoffQuickJourney({ onComplete, onSkipToDashboard }: Props) {
  ...
  const next = useCallback(() => {
    if (step === 0) {
      trackToolEvent(DEBT_PAYOFF_SLUG, "journey_start", undefined, true);
    }
      ...
      <div className="transition-opacity duration-300" key={step}>
            ...
            <div className="relative text-center">
              <div className="relative mx-auto mb-8 flex h-36 w-36 items-center justify-center sm:h-44 sm:w-44">
                ...
                <div className="relative flex h-full w-full flex-col items-center justify-center gap-2 rounded-[2rem] border border-zinc-200/90 bg-white/95 shadow-2xl ring-1 ring-emerald-900/5 backdrop-blur-sm dark:border-zinc-700 dark:bg-zinc-950/95 dark:ring-emerald-400/10">
                  <Layers className="h-16 w-16 text-emerald-700 dark:text-emerald-400" strokeWidth={1.2} aria-hidden />
                </div>
              </div>
        ...
        {step === 1 && (
          <WizardSlideShell
            ...
            subtitle="Your focus shapes labels on your snapshot — you can switch strategies anytime in the full planner."
            onBack={back}
            onNext={next}
          ...
          >
            <div className="space-y-6">
              ...
              <input
                ...
                aria-label="Debt 1 balance"
                className="w-full h-3 accent-zinc-900 dark:accent-zinc-100 rounded-full"
              />
              <div className="grid sm:grid-cols-2 gap-4">
                <label className="block space-y-1">
                  <span className="text-xs font-semibold uppercase text-zinc-500 dark:text-zinc-400">APR %</span>
                  <input
                    type="number"
                    step={0.1}
                    min={0}
                    ...
                    aria-label="Debt 1 APR"
                    className="w-full rounded-xl border border-zinc-200 bg-white px-3 py-2 font-semibold dark:border-zinc-700 dark:bg-zinc-950 focus:outline-none focus-visible:ring-2 focus-visible:ring-emerald-400 focus-visible:ring-offset-2"
                  />
                </label>
                <label className="block space-y-1">
                  <span className="text-xs font-semibold uppercase text-zinc-500 dark:text-zinc-400">Minimum / mo</span>
                  <input
                    type="number"
                    min={0}
                    max={5000}
                    ...
                    aria-label="Debt 1 minimum payment"
                    className="w-full rounded-xl border border-zinc-200 bg-white px-3 py-2 font-semibold dark:border-zinc-700 dark:bg-zinc-950 focus:outline-none focus-visible:ring-2 focus-visible:ring-emerald-400 focus-visible:ring-offset-2"
                  />
                </label>
              </div>
              ...
              <div className="grid sm:grid-cols-2 gap-4">
                ...
                <label className="block space-y-1">
                  ...
                  <input
                    ...
                    aria-label="Debt 2 minimum payment"
                    className="w-full rounded-xl border border-zinc-200 bg-white px-3 py-2 font-semibold dark:border-zinc-700 dark:bg-zinc-950 focus:outline-none focus-visible:ring-2 focus-visible:ring-emerald-400 focus-visible:ring-offset-2"
                  />
                </label>
              </div>
          ...
          >
            <div className="space-y-6">
              ...
              <input
                ...
                aria-label="Extra monthly payment toward debt"
                className="w-full h-3 accent-zinc-900 dark:accent-zinc-100 rounded-full"
              />
              <label className="block">
                ...
                <input
                  ...
                  aria-label="Exact extra monthly payment"
                  className="mt-2 w-full rounded-xl border border-zinc-200 bg-white px-4 py-3 text-lg font-semibold dark:border-zinc-700 dark:bg-zinc-950 focus:outline-none focus-visible:ring-2 focus-visible:ring-emerald-400 focus-visible:ring-offset-2"
                />
              </label>
            </div>
          </WizardSlideShell>
