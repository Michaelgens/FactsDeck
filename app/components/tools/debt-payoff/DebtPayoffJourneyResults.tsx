...
import {
  buildDebtTextSummary,
  computeDebtPayoffJourneyMetrics,
  computeDebtReadinessScore,
  formatDebtMoney,
  suggestRelatedTools,
} from "./compute-debt-payoff-metrics";
...
import DebtPayoffRelatedTools from "./DebtPayoffRelatedTools";
...
export default function DebtPayoffJourneyResults({ answers, onOpenDashboard, onStartOver }: Props) {
  ...
  const readinessScore = computeDebtReadinessScore(answers);
  const relatedTools = suggestRelatedTools(answers.goal, answers);
  const [copied, setCopied] = useState(false);

  const summaryText = buildDebtTextSummary(answers, m);

  const copySummary = useCallback(async () => {
    try {
      await navigator.clipboard.writeText(summaryText);
      trackToolEvent(DEBT_PAYOFF_SLUG, "export_text");
      setCopied(true);
  ...
  const highlightStrategy = answers.goal === "snowball" ? "snowball" : answers.goal === "avalanche" ? "avalanche" : null;
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-8 sm:py-14 space-y-8 sm:space-y-10 pb-28 sm:pb-14">
        <div className="rounded-3xl border border-zinc-200 bg-white p-6 sm:p-10 shadow-sm dark:border-zinc-800 dark:bg-zinc-900/40">
          ...
          <p className="mt-2 font-display text-4xl sm:text-6xl font-bold tabular-nums tracking-tight text-zinc-900 dark:text-zinc-50">
            {fmtMo(m.avalancheMonths, m.avalanchePaidOff)}
            <span className="text-lg sm:text-xl font-semibold text-zinc-500 dark:text-zinc-400"> avalanche</span>
          </p>
          <div className="mt-3 flex flex-wrap items-center gap-2 text-sm text-zinc-600 dark:text-zinc-400">
            <span>Snowball {fmtMo(m.snowballMonths, m.snowballPaidOff)}</span>
            ...
            <span className="inline-flex items-center rounded-full border border-zinc-200 bg-zinc-50 px-2.5 py-1 text-xs font-semibold text-zinc-700 dark:border-zinc-800 dark:bg-zinc-950 dark:text-zinc-200">
              Extra {formatDebtMoney(answers.extraMonthly)}/mo
            </span>
          </div>
            ...
            <div className="flex gap-3 overflow-x-auto pb-2 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden sm:grid sm:grid-cols-3 sm:gap-4 sm:overflow-visible sm:pb-0">
              <div
                className={`min-w-[14.5rem] sm:min-w-0 rounded-2xl bg-zinc-50 dark:bg-zinc-950/80 p-5 border ${
                  highlightStrategy === "snowball"
                    ? "ring-2 ring-emerald-500/60 dark:ring-emerald-400/50 border-emerald-200 dark:border-emerald-800"
                    : "border-zinc-100 dark:border-zinc-800"
                }`}
              >
                <p className="text-xs font-bold uppercase text-zinc-500 dark:text-zinc-400">Snowball</p>
                <p className="mt-2 text-2xl font-bold tabular-nums">{fmtMo(m.snowballMonths, m.snowballPaidOff)}</p>
                <p className="mt-1 text-xs text-zinc-500 dark:text-zinc-400">Interest ≈ {fmtInterest(m.snowballInterest)}</p>
              </div>
              <div
                className={`min-w-[14.5rem] sm:min-w-0 rounded-2xl bg-zinc-50 dark:bg-zinc-950/80 p-5 border ${
                  highlightStrategy === "avalanche"
                    ? "ring-2 ring-emerald-500/60 dark:ring-emerald-400/50 border-emerald-200 dark:border-emerald-800"
                    : "border-zinc-100 dark:border-zinc-800"
                }`}
              >
                <p className="text-xs font-bold uppercase text-zinc-500 dark:text-zinc-400">Avalanche</p>
                <p className="mt-2 text-2xl font-bold tabular-nums">{fmtMo(m.avalancheMonths, m.avalanchePaidOff)}</p>
                <p className="mt-1 text-xs text-zinc-500 dark:text-zinc-400">Interest ≈ {fmtInterest(m.avalancheInterest)}</p>
              </div>
              <div className="min-w-[14.5rem] sm:min-w-0 rounded-2xl bg-zinc-50 dark:bg-zinc-950/80 p-5 border border-zinc-100 dark:border-zinc-800">
                <p className="text-xs font-bold uppercase text-zinc-500 dark:text-zinc-400">Interest Δ (av − snow)</p>
                <p className="mt-2 text-2xl font-bold tabular-nums">{fmtInterest(m.interestDelta)}</p>
                <p className="mt-1 text-xs text-zinc-500 dark:text-zinc-400">Months Δ: {m.monthsDelta} mo</p>
              </div>
            </div>
            <p className="mt-2 text-xs text-zinc-500 dark:text-zinc-400 sm:hidden">Swipe to see all strategies</p>
          </div>
        </div>
        ...
        <div className="rounded-3xl border border-zinc-200 bg-white p-6 sm:p-8 shadow-sm dark:border-zinc-800 dark:bg-zinc-900/40">
          ...
          <p className="mt-2 text-sm text-zinc-600 dark:text-zinc-400">
            Based on extra payment vs minimums, APR mix, and total debt size.
          </p>
        </div>
        ...
        <div className="grid md:grid-cols-2 gap-4 sm:gap-6">
          <div className="rounded-3xl border-2 border-zinc-900 bg-zinc-900 p-6 sm:p-8 text-white dark:border-zinc-100 dark:bg-zinc-100 dark:text-zinc-900">
            <div className="flex items-center gap-2 mb-3">
              ...
              <h2 className="font-display text-xl font-bold">Full debt payoff planner</h2>
            </div>
        ...
        {relatedTools.length > 0 ? (
          <div className="max-w-xl">
            <DebtPayoffRelatedTools tools={relatedTools} />
          </div>
        ) : null}
