...
import {
  buildCreditTextSummary,
  computeCreditJourneyMetrics,
  computeCreditReadinessScore,
  suggestRelatedTools,
} from "./compute-credit-journey-metrics";
import type { CreditJourneyAnswers } from "./credit-journey-types";
import { FACTS_DECK_CREDIT_TEST } from "./credit-journey-types";
import CreditRelatedTools from "./CreditRelatedTools";
...
const FACTOR_KEYS = ["utilization", "payment", "age", "inquiries", "mix"] as const;
const FACTOR_SHORT: Record<(typeof FACTOR_KEYS)[number], string> = {
  utilization: "Util",
  payment: "Pay",
  age: "Age",
  inquiries: "Inq",
...
export default function CreditJourneyResults({ answers, onOpenDashboard, onStartOver }: Props) {
  ...
  const readinessScore = computeCreditReadinessScore(m);
  const relatedTools = suggestRelatedTools(answers.goal, answers);
  const [copied, setCopied] = useState(false);

  const summaryText = buildCreditTextSummary(answers, m);

  const copySummary = useCallback(async () => {
    try {
      await navigator.clipboard.writeText(summaryText);
      trackToolEvent(CREDIT_SCORE_SLUG, "export_text");
      setCopied(true);
  ...
  return (
    <div className="relative min-h-screen overflow-x-hidden bg-zinc-50 dark:bg-zinc-950 text-zinc-900 dark:text-zinc-100">
      ...
      />
      <div
        className="pointer-events-none absolute -top-32 left-1/2 h-[42rem] w-[min(90rem,200%)] -translate-x-1/2 rounded-full bg-gradient-to-b from-blue-200/35 via-orange-100/15 to-transparent blur-3xl dark:from-violet-950/50 dark:via-indigo-950/30 dark:to-transparent"
        aria-hidden
      />
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-8 sm:py-14 space-y-8 sm:space-y-10 pb-28 sm:pb-14">
        <div className="rounded-3xl border border-zinc-200 bg-white p-6 sm:p-10 shadow-sm dark:border-zinc-800 dark:bg-zinc-900/40">
          ...
          <div className="mt-3 flex flex-wrap items-center gap-2 text-sm text-zinc-600 dark:text-zinc-400">
            <span className="font-semibold text-zinc-800 dark:text-zinc-200">{m.bandName}</span>
            ...
            <span className="inline-flex items-center rounded-full border border-zinc-200 bg-zinc-50 px-2.5 py-1 text-xs font-semibold text-zinc-700 dark:border-zinc-800 dark:bg-zinc-950 dark:text-zinc-200">
              Util {answers.utilizationPct}%
            </span>
            <span className="inline-flex items-center rounded-full border border-zinc-200 bg-zinc-50 px-2.5 py-1 text-xs font-semibold text-zinc-700 dark:border-zinc-800 dark:bg-zinc-950 dark:text-zinc-200">
              On-time {answers.onTimePct}%
            </span>
          </div>
          ...
          <div className="mt-6 sm:mt-8 -mx-6 px-6 sm:mx-0 sm:px-0">
            <div className="flex gap-3 overflow-x-auto pb-2 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden sm:grid sm:grid-cols-5 sm:gap-3 sm:overflow-visible sm:pb-0">
              {FACTOR_KEYS.map((k) => (
                <div
                  key={k}
                  className="min-w-[5.5rem] sm:min-w-0 shrink-0 sm:shrink rounded-xl bg-zinc-50 p-3 text-center dark:bg-zinc-950/80 border border-zinc-100 dark:border-zinc-800"
                >
                  <p className="text-xs font-bold text-zinc-500 dark:text-zinc-400">{FACTOR_SHORT[k]}</p>
                  <p className="mt-1 text-lg font-semibold tabular-nums">{m.pts[k]}</p>
                  <p className="text-[10px] text-zinc-400 dark:text-zinc-500">pts</p>
                </div>
              ))}
            </div>
            <p className="mt-2 text-xs text-zinc-500 dark:text-zinc-400 sm:hidden">Swipe to see all factors</p>
          </div>
        </div>
        ...
        <div className="rounded-3xl border border-zinc-200 bg-white p-6 sm:p-8 shadow-sm dark:border-zinc-800 dark:bg-zinc-900/40">
          ...
          <p className="mt-2 text-sm text-zinc-600 dark:text-zinc-400">
            Normalized position on the 300–850 illustrative scale.
          </p>
        </div>
        ...
        {relatedTools.length > 0 ? (
          <div className="max-w-xl">
            <CreditRelatedTools tools={relatedTools} />
          </div>
        ) : null}
