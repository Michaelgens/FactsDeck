...
import { ArrowLeft, Coins, Compass, DoorOpen, Sparkles, Zap } from "lucide-react";
...
export default function CryptoYieldQuickJourney({ onComplete, onSkipToDashboard }: Props) {
  ...
  const next = useCallback(() => {
    if (step === 0) {
      trackToolEvent(CRYPTO_YIELD_SLUG, "journey_start", undefined, true);
    }
  ...
  return (
    <div className="relative overflow-x-hidden overflow-y-hidden bg-white dark:bg-zinc-950 text-zinc-900 dark:text-zinc-100">
      ...
      />
      <div
        className="pointer-events-none absolute top-[28rem] right-[-10%] h-96 w-96 rounded-full bg-amber-100/30 blur-3xl dark:bg-amber-950/25"
        aria-hidden
      />
          ...
          >
            <div className="relative text-center">
              <div className="relative mx-auto mb-8 flex h-36 w-36 items-center justify-center sm:h-44 sm:w-44">
                <div className="absolute -inset-6 rounded-full bg-gradient-to-br from-amber-400/30 via-orange-400/15 to-emerald-400/20 blur-2xl dark:from-amber-500/20 dark:via-orange-500/10" />
                ...
                <div className="relative flex h-full w-full flex-col items-center justify-center gap-2 rounded-[2rem] border border-zinc-200/90 bg-white/95 shadow-2xl ring-1 ring-amber-900/5 backdrop-blur-sm dark:border-zinc-700 dark:bg-zinc-950/95 dark:ring-amber-400/10">
                  <Coins className="h-16 w-16 text-amber-700 dark:text-amber-400" strokeWidth={1.2} aria-hidden />
                </div>
              </div>
            ...
            <div className="space-y-8">
              <div>
                ...
                <input
                  ...
                  className="mt-4 w-full h-3 accent-zinc-900 dark:accent-zinc-100 rounded-full focus:outline-none focus-visible:ring-2 focus-visible:ring-amber-400"
                />
              </div>
              <div>
                ...
                <input
                  ...
                  aria-label="Nominal APY percent"
                  className="mt-4 w-full h-3 accent-zinc-900 dark:accent-zinc-100 rounded-full focus:outline-none focus-visible:ring-2 focus-visible:ring-amber-400"
                />
              </div>
            </div>
            ...
            <div className="space-y-8">
              <div>
                ...
                <input
                  ...
                  aria-label="Investment horizon in months"
                  className="w-full h-3 accent-zinc-900 dark:accent-zinc-100 rounded-full focus:outline-none focus-visible:ring-2 focus-visible:ring-amber-400"
                />
              </div>
              <div>
                <p className="text-sm font-semibold text-zinc-700 dark:text-zinc-300 mb-3">Compounding</p>
                <div className="flex flex-wrap gap-2" role="group" aria-label="Compounding frequency">
                  {(["daily", "monthly", "annual"] as const).map((c) => (
                    <button
                      key={c}
                      type="button"
                      onClick={() => setField("compounding", c)}
                      aria-pressed={a.compounding === c}
                      className={`rounded-full px-4 py-2 text-sm font-bold border transition-colors focus:outline-none focus-visible:ring-2 focus-visible:ring-amber-400 focus-visible:ring-offset-2 ${
                        a.compounding === c
