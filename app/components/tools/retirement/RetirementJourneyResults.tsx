...
import {
  buildRetirementTextSummary,
  computeRetirementJourneyMetrics,
  computeRetirementReadinessScore,
  formatRetireMoney,
  suggestRelatedTools,
} from "./compute-retirement-journey-metrics";
...
import RetirementRelatedTools from "./RetirementRelatedTools";
...
export default function RetirementJourneyResults({ answers, onOpenDashboard, onStartOver }: Props) {
  ...
  const readinessScore = computeRetirementReadinessScore(answers, m);
  const relatedTools = suggestRelatedTools(answers.goal, answers);
  const [copied, setCopied] = useState(false);

  const summaryText = buildRetirementTextSummary(answers, m);

  const copySummary = useCallback(async () => {
    try {
      await navigator.clipboard.writeText(summaryText);
      trackToolEvent(RETIREMENT_SLUG, "export_text");
      setCopied(true);
  ...
  const heroMetric = answers.goal === "fire" ? m.balanceAtRetire : m.fiNumber;
  const heroLabel = answers.goal === "fire" ? "Projected balance at retire" : "FI number (simple)";
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-8 sm:py-14 space-y-8 sm:space-y-10 pb-28 sm:pb-14">
        <div className="rounded-3xl border border-zinc-200 bg-white p-6 sm:p-10 shadow-sm dark:border-zinc-800 dark:bg-zinc-900/40">
          ...
          <div className="mt-3 flex flex-wrap items-center gap-2 text-sm text-zinc-600 dark:text-zinc-400">
            ...
            >
              {m.onTrack ? "On track" : `${formatRetireMoney(m.gap)} below target`}
            </span>
          </div>
          ...
          <div className="mt-6 sm:mt-8 -mx-6 px-6 sm:mx-0 sm:px-0">
            <div className="flex gap-3 overflow-x-auto pb-2 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden sm:grid sm:grid-cols-3 sm:gap-4 sm:overflow-visible sm:pb-0">
              <div className="min-w-[14.5rem] sm:min-w-0 rounded-2xl bg-zinc-50 dark:bg-zinc-950/80 p-5 border border-zinc-100 dark:border-zinc-800">
                ...
                <p className="mt-2 text-2xl font-bold tabular-nums">{formatRetireMoney(m.fiNumber)}</p>
                <p className="mt-1 text-xs text-zinc-500 dark:text-zinc-400">
                  @ {answers.withdrawalRate.toFixed(1)}% withdrawal
                </p>
              </div>
              <div className="min-w-[14.5rem] sm:min-w-0 rounded-2xl bg-zinc-50 dark:bg-zinc-950/80 p-5 border border-zinc-100 dark:border-zinc-800">
                <p className="text-xs font-bold uppercase text-zinc-500 dark:text-zinc-400">Balance at retire</p>
                <p className="mt-2 text-2xl font-bold tabular-nums">{formatRetireMoney(m.balanceAtRetire)}</p>
                <p className="mt-1 text-xs text-zinc-500 dark:text-zinc-400">
                  Spend ~{formatRetireMoney(m.spendAtRetire)}/yr
                </p>
              </div>
              <div className="min-w-[14.5rem] sm:min-w-0 rounded-2xl bg-zinc-50 dark:bg-zinc-950/80 p-5 border border-zinc-100 dark:border-zinc-800">
        ...
        <div className="grid md:grid-cols-2 gap-4 sm:gap-6">
          ...
          <div className="rounded-3xl border border-zinc-200 bg-white p-6 sm:p-8 shadow-sm dark:border-zinc-800 dark:bg-zinc-900/40">
            ...
            <p className="text-sm text-zinc-600 dark:text-zinc-400 mb-4">Plain-text snapshot for your notes or sharing.</p>
            <button
              type="button"
        ...
        {relatedTools.length > 0 ? (
          <div className="max-w-xl">
            <RetirementRelatedTools tools={relatedTools} />
          </div>
        ) : null}
