...
import {
  buildLoanTextSummary,
  computeLoanJourneyMetrics,
  computeLoanReadinessScore,
  formatLoanMoney,
  suggestRelatedTools,
} from "./compute-loan-journey-metrics";
import type { LoanJourneyAnswers } from "./loan-journey-types";
import { FACTS_DECK_LOAN_TEST } from "./loan-journey-types";
import LoanRelatedTools from "./LoanRelatedTools";
...
export default function LoanJourneyResults({ answers, onOpenDashboard, onStartOver }: Props) {
  ...
  const readinessScore = computeLoanReadinessScore(answers);
  const relatedTools = suggestRelatedTools(answers.goal, answers);
  const [copied, setCopied] = useState(false);

  const summaryText = buildLoanTextSummary(answers, m);

  const copySummary = useCallback(async () => {
    try {
      await navigator.clipboard.writeText(summaryText);
      trackToolEvent(LOAN_SLUG, "export_text");
      setCopied(true);
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-8 sm:py-14 space-y-8 sm:space-y-10 pb-28 sm:pb-14">
        <div className="rounded-3xl border border-zinc-200 bg-white p-6 sm:p-10 shadow-sm dark:border-zinc-800 dark:bg-zinc-900/40">
          ...
          <div className="mt-3 flex flex-wrap items-center gap-2 text-sm text-zinc-600 dark:text-zinc-400">
            <span>Principal {formatLoanMoney(answers.principal)}</span>
            ...
            <span className="inline-flex items-center rounded-full border border-zinc-200 bg-zinc-50 px-2.5 py-1 text-xs font-semibold text-zinc-700 dark:border-zinc-800 dark:bg-zinc-950 dark:text-zinc-200">
              {answers.apr.toFixed(2)}% APR
            </span>
            <span className="inline-flex items-center rounded-full border border-zinc-200 bg-zinc-50 px-2.5 py-1 text-xs font-semibold text-zinc-700 dark:border-zinc-800 dark:bg-zinc-950 dark:text-zinc-200">
              {answers.termYears} yr
            </span>
          </div>
        ...
        {relatedTools.length > 0 ? (
          <div className="max-w-xl">
            <LoanRelatedTools tools={relatedTools} />
          </div>
        ) : null}
